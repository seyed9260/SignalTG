#!/bin/bash 
# running launchs.sh with sudo privileges.
sudo launchs.sh
clear
echo -e "\033[38;5;208m"
echo -e " Sudo Team - Sudo Team - Sudo Team - Sudo Team "
echo -e " Sudo Team - Sudo Team - Sudo Team - Sudo Team "
echo -e " Sudo Team - Sudo Team - Sudo Team - Sudo Team "
echo -e " Sudo Team - Sudo Team - Sudo Team - Sudo Team "
echo -e "                                              \033[0;00m"
echo -e "          Developed By : @negative_officiall , Edited By : @FireBlackHat       "
echo -e "                                               "
echo -e "\e[34m"
...
if [ "$VAR" = 1 ]; then
	...
	clear
	mkdir /home/SudoBackup
	rm -R /home/SudoBackup/SudoTM
	clear
	cp -R ../SignalTG/ /home/SudoBackup
	echo "Respaldo exitoso! Guardado en /home/DBTeamBackup."
	read -n1 -r -p 'Si quieres borrar los backups escribe bkpdel al iniciar launchs.sh Presiona cualquier tecla para finalizar'
	clear
elif [ "$VAR" = 8 ]; then
	clear
	killall screen
	...
	echo "            Si deseas eliminar todos los backups escribe bkpdel al iniciar launchs.sh"
...
read subVAR
	if [ "$subVAR"  = y ]; then
		mkdir /home/SudoBackup
		rm -R /home/SudoBackup/plugins
		clear
		cp -R plugins/ /home/SudoBackup
		clear
		echo -e "\e[32m"
		dpkg -s subversion 2>/dev/null >/dev/null || sudo apt-get -y install subversion
		rm -rf plugins
		svn export https://github.com/SIGNALTEAM/SignalTG/tree/supergroups/plugins
...
if [ "$VAR" = 1 ]; then
	...
	clear
	cp -R ../SignalTG/ /home/SudoBackup
	echo "Backup finished! Saved in /home/SudoBackup."
	read -n1 -r -p 'If you want delete all backups type bkpdel at run launchs.sh Press any key to exit'
	clear
elif [ "$VAR" = 8 ]; then
	clear
	killall screen
	...
	echo "           Any Modifications will be replaced with the SudoTM repository."
	echo "           launchs.sh will make a backup in /home/SudoBackup/plugins."
	echo "           If you want delete all backups, type bkpdel at run launchs.sh"
	echo "Are you sure?"
	echo "	Yes = y			Not= n		"
# SubVar confirmation and change color
echo -e "\e[32m"
...
# end
	...
	clear
	echo -e "      SudoTM installation will start.      "
	echo -e "SudoTM was developed by @Josepdal and @MaSkAoS"
	echo -e "Thanks to @iicc1 and @Jarriz because SudoTM works easily and more stable."
	...
	./launch.sh install
	read -n1 -r -p 'Step 3/3. Instalation finished! Thanks to install SudoTM, the team of SudoTM say thank you! We hope you like SudoTM. Then, the script will ask your number y and the confirmation code, Telegram will send a sms o Telegram msg, please, type your numer with + [Code state] [Your phone]'
	clear
	service redis-server start
	clear
	./launch.sh
...
if [ "$VAR" = 1 ]; then
	...
	clear
	cp -R ../SignalTG/ /home/SudoBackup
	echo "Backup finalizado! Salvo em /home/SudoBackup."
	read -n1 -r -p 'If you want delete all backups type bkpdel at run launchs.sh Pressione qualquer tecla para finalizar.'
	clear
elif [ "$VAR" = 8 ]; then
	clear
	killall screen
...
if [ "$BKPVAR" = 1 ]; then
	...
	clear
	echo -e "\e[31m"
	echo "Backups of SudoTM removed"
	echo -e "\e[32m"
elif [ "$BKPVAR" = 3 ]; then
	rm -R /home/SudoBackup/
	clear
	echo -e "\e[31m"
